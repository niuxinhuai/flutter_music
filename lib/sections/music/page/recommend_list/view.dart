...
Widget buildView(
  RecommendListState state,
  Dispatch dispatch,
  ViewService viewService,
) {
  return Scaffold(
    backgroundColor: CommonColors.foregroundColor,
    appBar: GpAppBar(elevation: 0, title: Text("每日推荐")),
    body: LoadingWrap(
    ...
    child: Container(
      ...
      decoration: BoxDecoration(
        image: DecorationImage(
          image: NetworkImage(
            state.wrap!.data!.dailySongs!.first.al?.picUrl ?? "",
          ),
          fit: BoxFit.fill,
        ),
      ),
...
Widget _buildList(
  ...
  return SliverList(
    delegate: SliverChildBuilderDelegate((context, index) {
      ...
      return SongItemWidget(item: item, dispatch: dispatch);
